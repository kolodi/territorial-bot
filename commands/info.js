...
const execute = async (interaction, db, cache, client, totalCommandCounter) => {
  ...
  interaction.reply({ content: 'This command gives you different information about the bot.', embeds: [ie], components: [btnRow] }).then(msg => {
      ...
      collecter.on('collect', async i => {
        if (i.customId === 'info') {
          i.update({ content: 'Showing **Normal Info**', embeds: [ie], components: [btnRow] })
        }
        if (i.customId === 'uptime') {
          i.update({ content: 'You have pressed another button!', embeds: [ue], components: [btnRow] })
        }
        if (i.customId === 'ping') {
          i.update({ content: 'You have pressed a button!', embeds: [pe], components: [btnRow] })
        }
      })
